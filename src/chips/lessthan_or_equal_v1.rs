...
impl<F: Field + Ord, const N_BYTES: usize> LtEqVecChip<F, N_BYTES> {
    ...
    pub fn configure(
        ...
        meta.create_gate("lt gate", |meta| {
            ...
            let check_a = lhs(meta) - (rhs(meta) + one_expression) - expr_from_bytes(&diff_bytes)
                + (lt.clone() * range);
...
impl<F: Field + Ord, const N_BYTES: usize> LtEqVecInstruction<F> for LtEqVecChip<F, N_BYTES> {
    fn assign_right_constant(
        ...
        for i in 0..lhs.len() {
            ...
            let diff = (lhs[i] - (rhs + F::ONE)) + (if temp { config.range } else { F::ZERO });
